...
class LLMAgent(BaseAgent):
    ...
    def _build_prompt(
        ...
        game_rules = {
            ...
            "payoff_formula": (
                "payoff_i = endowment - own_contribution "
                "+ multiplier * total_contribution / number_of_agents"
            ),
            "social_dilemma": (
                "Group welfare increases with cooperation, but individual free-riding "
                "can increase short-term payoff."
            ),
        }
        ...
        if self.mode == "tom":
            task = (
                "Use functional Theory of Mind. Infer whether sampled agents are cooperative, "
                "self-interested, or adaptive; infer what they may believe about the group; "
                "predict their next contribution; then choose your own contribution. "
                "Optimize long-run payoff, not only immediate payoff."
            )
                ...
                "predictions": [
                    {
                        ...
                        "inferred_type": (
                            "cooperative/self-interested/conditional/adaptive/unknown"
                        ),
                        "inferred_belief": (
                            "short phrase about what the target likely believes about the group"
                        ),
                    }
                ],
            ...
            task = (
                "Choose your next contribution based on the game history summary. "
                "Do not explicitly discuss beliefs, intentions, or mental states of other agents. "
                "You may make numeric predictions only if useful."
            )
    ...
    def _parse_decision(
        ...
        contribution = clamp_int(
            parsed.get("own_contribution"),
            0,
            config.endowment,
            default=config.endowment // 2,
        )
        ...
        if isinstance(pred_rows, list):
            for row in pred_rows:
                ...
                pred_val = clamp_float(
                    row.get("predicted_contribution"),
                    0,
                    config.endowment,
                    default=None,
                )
